...
To address RQ1 and RQ3's investigation of quantization effects on RAG performance, we fine-tuned three versions of LLaMA-2-7B~\cite{touvron2023llama} on conversation summarization tasks. We merged SAMSum~\cite{gliwa2019samsum} (16,369 conversations) and DialogSum~\cite{chen2021dialogsum} (13,460 dialogues), creating 28,982 examples split into 80\% training, 10\% validation, and 10\% test sets.

Dataset preprocessing followed standard instruction-following format with system prompt, user dialogue, and assistant summary. We tokenized using LLaMA-2's SentencePiece tokenizer and applied model-specific truncation. For conversations exceeding sequence limits, we used hierarchical summarization with 512-token sliding windows. We masked input tokens during loss calculation, computing cross-entropy only on summary tokens to prevent verbatim copying.

We implemented quantized fine-tuning using QLoRA~\cite{dettmers2023qlora} with LoRA~\cite{hu2022lora} via PEFT~\cite{peft2023} and BitsAndBytes~\cite{dettmers2022bitfit}. QLoRA's 4-bit NF4 quantization with double quantization reduces memory from 28GB (full-precision) to 4-5GB while maintaining performance. We applied LoRA to all attention and feedforward layers, reducing trainable parameters from 7B to 16-32M (0.2-0.5\%).

Table~\ref{tab:hyperparameters} details three model versions. \textbf{v1}: no quantization, 18.2GB memory. \textbf{v2}: 8-bit, 8.1GB. \textbf{v3}: 4-bit, 4.5GB (75\% reduction), adjusted learning rate and epochs for stability.
...
To address RQ4, we implemented hybrid retrieval combining BM25 and semantic search via Reciprocal Rank Fusion (RRF). This approach captures complementary strengths: BM25 for exact term matching, semantic search for paraphrases.

The lexical search component employs BM25~\cite{robertson2009bm25} with parameters $k_1=1.5$ and $b=0.75$ following TREC best practices. We index 50,000 document chunks (512 tokens with 50-token overlap) using Elasticsearch 8.11, retrieving top-10 chunks in 15-25ms.

The semantic search component uses BAAI/bge-small-en-v1.5~\cite{bge2023} to generate 384-dimensional embeddings. We store embeddings in Qdrant~\cite{qdrant2023} vector database, achieving 8-12ms query latency via HNSW approximate nearest neighbor search, returning top-10 results filtered by 0.5 similarity threshold.
...
For RQ1 and RQ3, we measure model quality (training/validation loss via cross-entropy, ROUGE-L F1-score) and resource efficiency (GPU memory via NVML, inference speed in tokens/second). We apply early stopping when validation loss increases, and use standard ROUGE-1.5.5 implementation with stemming.
...
Table~\ref{tab:implementation} details our implementation environment. We conducted experiments on an NVIDIA RTX 4090 (24GB) with Intel i9-13900K CPU and 64GB RAM running Ubuntu 24.04 LTS with CUDA 12.1. The 4-bit model (v3) was validated on RTX 3050 8GB laptops, confirming consumer hardware accessibility. Our software stack uses PyTorch 2.1.0, Transformers 4.36.0, PEFT 0.8.0, and BitsAndBytes 0.41.0. We documented dependencies in \texttt{requirements.txt} and \texttt{Dockerfile} for reproducibility~\cite{paleyes2022mle}.

To ensure reproducibility, all experiments use seed=42 and deterministic algorithms (\texttt{torch.use\_deterministic\_algorithms(True)}). We tracked experiments using MLflow~\cite{chen2020mlflow}, logging hyperparameters and model checkpoints with Git hashes. The replication package described in Section~\ref{sec:replication} includes trained weights.
...
\noindent
Our findings contrast with QLoRA's original evaluation~\cite{dettmers2023qlora}, which reported 4-bit performance \textit{matching} full-precision on generic benchmarks (MMLU, HumanEval). We demonstrate 4-bit \textit{outperforming} full-precision specifically in fine-tuning scenarios with limited data (25,500 examples). This suggests quantization's regularization benefit emerges most strongly when overfitting risk is high—precisely the regime where practitioners need efficient fine-tuning most.
...
\begin{table}[tb]
    \caption{GPU scaling analysis for v3 (4-bit) fine-tuning and inference. Minimum requirement: 8GB VRAM. Training times assume full fine-tuning (1000 steps, batch size 4).}
    \label{tab:gpu-scaling}
    \centering
    \small
    \begin{tabular}{lcccc}
        \toprule
        \textbf{GPU Model} & \textbf{VRAM} & \textbf{Training Time} & \textbf{Inference} & \textbf{Cost/Hour} \\
        \midrule
        \multicolumn{5}{l}{\textit{Consumer GPUs}} \\
        RTX 3050 & 8 GB & 12 hours & 62 tok/s & \$0 (owned) \\
        RTX 4060 Ti & 16 GB & 9 hours & 78 tok/s & \$0 (owned) \\
        RTX 4090 & 24 GB & 8 hours & 95 tok/s & \$0 (owned) \\
        \midrule
        \multicolumn{5}{l}{\textit{Cloud GPUs (AWS)}} \\
        V100 (p3.2xlarge) & 16 GB & 10 hours & 71 tok/s & \$3.06 \\
        A100 (p4d.24xlarge) & 40 GB & 7 hours & 105 tok/s & \$32.77 \\
        \botrule
    \end{tabular}
\end{table}

\noindent
Table~\ref{tab:gpu-scaling} demonstrates v3's accessibility across hardware tiers. Entry-level consumer GPUs (RTX 3050 8GB, \$250) successfully complete fine-tuning in 12 hours, democratizing LLM adaptation for individual researchers and small teams. Mid-range GPUs (RTX 4060 Ti) reduce training time to 9 hours while maintaining zero marginal cost for owned hardware. Cloud deployments on V100 instances cost \$30.60 for full fine-tuning (10 hours $\times$ \$3.06/hour), competitive with commercial API fine-tuning services (\$50-100 typical). The A100's 7-hour training time benefits large-scale experiments requiring rapid iteration but increases cost proportionally (\$229 per run).
...
\begin{table}[tb]
    \caption{Manual evaluation of v3 (4-bit) generated summaries by three independent evaluators on 100 randomly sampled conversations. Krippendorff's $\alpha$ = 0.78 indicates substantial inter-rater agreement.}
    \label{tab:chat-metrics}
    \centering
    \small
    \begin{tabular}{lcp{6cm}}
        \toprule
        \textbf{Metric} & \textbf{Score} & \textbf{Definition} \\
        \midrule
        Accuracy & 89\% & Summary captures key facts without hallucinations \\
        Completeness & 84\% & All important points from conversation included \\
        Conciseness & 92\% & No redundant information, appropriate length \\
        \midrule
        \multicolumn{3}{l}{Inter-rater reliability: Krippendorff's $\alpha$ = 0.78} \\
        \botrule
    \end{tabular}
\end{table}

\noindent
To validate that ROUGE scores reflect genuine quality improvements rather than statistical artifacts, we conducted manual evaluation on 100 randomly sampled conversations from the test set. Three independent evaluators (graduate students in computer science, fluent in English) assessed v3-generated summaries across three dimensions: accuracy (factual correctness), completeness (coverage of key points), and conciseness (brevity without information loss). Table~\ref{tab:chat-metrics} shows strong performance across all metrics (84-92\%), confirming that quantization's performance gains translate to human-perceived quality. The substantial inter-rater agreement (Krippendorff's $\alpha$ = 0.78) validates evaluation reliability. High conciseness (92\%) particularly indicates that v3 avoids verbosity—a common failure mode in undertrained models.
...
\noindent
Table~\ref{tab:query-type-analysis} reveals that hybrid retrieval's benefit varies dramatically by query type, providing actionable insights for system designers. \textbf{Multi-hop queries} (e.g., "Explain how LoRA's rank parameter affects both training speed and model capacity") benefit most: hybrid P@5 of 0.75 represents 70\% improvement over BM25 (0.44) and 10\% over semantic search (0.68). These queries require combining lexical anchor terms ("LoRA", "rank parameter") with semantic understanding of relationships ("affects both X and Y"). BM25 fails to connect concepts, while semantic-only struggles with technical terminology. \textbf{Conceptual queries} (e.g., "Explain the relationship between quantization and regularization") show 4\% hybrid gain: semantic search excels (0.83) since synonyms and paraphrases dominate, but BM25 struggles (0.52) due to lexical variation. \textbf{Factual queries} (e.g., "What is the default learning rate?") exhibit smallest gains: hybrid achieves 0.87 versus BM25's 0.81 (7\% improvement). Exact term matching suffices for fact lookup, limiting semantic search's additive value.

\noindent
The mechanism underlying hybrid retrieval's success relates to complementary failure modes. BM25 excels when query terms appear verbatim in documents but fails when synonyms, pronouns, or conceptual relationships require bridging. Semantic search handles paraphrasing robustly through dense embeddings but occasionally misranks documents with high lexical overlap (e.g., shared boilerplate text) that lack semantic relevance. RRF fusion~\cite{cormack2009rrf} mitigates both failure modes by promoting documents that rank highly in \textit{either} retrieval method, effectively implementing an OR operation over lexical and semantic match signals.

\noindent
Comparison to learned fusion methods highlights RRF's practical advantages. Cross-encoder rerankers~\cite{nogueira2020monoT5} achieve 3-5\% higher precision on MS MARCO but require: (1) training data with query-document relevance labels (thousands of examples), (2) additional inference cost (BERT-base reranker adds 50-100ms latency), and (3) hyperparameter tuning (learning rate, number of epochs). RRF requires none of these—the constant $c=60$ transfers across domains without tuning. For production systems with evolving document corpora (where training data quickly becomes stale), RRF's parameter-free nature provides robust baseline performance.
...
\noindent
\textbf{Mitigation:} 
We supplemented ROUGE with manual evaluation by three independent evaluators (inter-rater reliability: Krippendorff's $\alpha$ = 0.78). Manual metrics (accuracy 89\%, completeness 84\%, conciseness 92\%) confirm high quality beyond lexical overlap (Table~\ref{tab:chat-metrics}). ROUGE remains primary metric due to scalability constraints.

\noindent
\textbf{Threat 2: Dataset representativeness.} 
Fine-tuning datasets (SAMSum~\cite{gliwa2019samsum}, DialogSum~\cite{chen2021dialogsum}) focus on conversational summarization, potentially limiting generalizability to other RAG tasks.
...
\noindent
\textbf{Residual Risk:} 
Performance on non-conversational RAG tasks remains unvalidated. Future work should evaluate on MSMARCO~\cite{bajaj2016msmarco}, Natural Questions~\cite{kwiatkowski2019natural}.
...
Evaluation primarily English; embedding model (bge-small-en-v1.5~\cite{xiao2024bge}) optimized for English.
...
\noindent
\textbf{Mitigation:} 
We provide scaling analysis for consumer GPUs and cloud instances in Table~\ref{tab:gpu-scaling}. Memory requirements (4.5GB v3) fit within 8GB budget.
...
\noindent
\textbf{Mitigation:} 
Precision@5 remained stable (0.79-0.83) across tested range (1K-50K). Qdrant HNSW index scales to millions of vectors~\cite{qdrant2021}.
